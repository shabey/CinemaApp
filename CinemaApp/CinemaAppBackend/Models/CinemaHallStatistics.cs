...
namespace CinemaAppBackend.Models
{
    public class CinemaHallStatistics
    {
        ...
        public override string ToString()
        {
            ...
            output.AppendLine();
            output.AppendLine($"Percentage occupied: {this.PercentageOccupied.ToString("P2")}");
            output.AppendLine();
            output.AppendLine($"Current income (sum of reserved tickets): {this.CurrentIncome.ToString("F2")}");
            output.AppendLine();
            output.AppendLine($"Potential total income (sum of all available and reserved tickets): {this.PotentialTotalIncome.ToString("F2")}");
